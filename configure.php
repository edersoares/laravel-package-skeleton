...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i vendor | findstr /v /i ' . basename(__FILE__) . ' | findstr /r /i /M /F:/ ":author :vendor :package VendorName VendorNameEscaped classname migration_table_name vendor_name vendor_slug author@domain.com"'));
}

function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('grep -E -r -l -i ":author|:vendor|:package|VendorName|VendorNameEscaped|classname|migration_table_name|vendor_name|vendor_slug|author@domain.com" --exclude-dir=vendor ./* ./.github/* | grep -v ' . basename(__FILE__)));
}
...
foreach ($files as $file) {
    ...
    match (true) {
        str_contains($file, determineSeparator('src/Providers/ClassNameServiceProvider.php')) => rename($file, determineSeparator('./src/Providers/' . $className . 'ServiceProvider.php')),
        str_contains($file, determineSeparator('src/Console/Commands/ClassNameCommand.php')) => rename($file, determineSeparator('./src/Console/Commands/' . $className . 'Command.php')),
        str_contains($file, determineSeparator('src/Http/Controllers/ClassNameController.php')) => rename($file, determineSeparator('./src/Http/Controllers/' . $className . 'Controller.php')),
        str_contains($file, determineSeparator('src/Models/ClassName.php')) => rename($file, determineSeparator('./src/Models/' . $className . '.php')),
        str_contains($file, determineSeparator('database/migrations/0000_00_00_000000_create_classname_table.php')) => rename($file, determineSeparator('./database/migrations/0000_00_00_000000_create_' . title_snake($packageSlugWithoutPrefix) . '_table.php')),
        str_contains($file, determineSeparator('database/factories/ClassNameFactory.php')) => rename($file, determineSeparator('./database/factories/' . $className . 'Factory.php')),
        str_contains($file, determineSeparator('database/seeders/ClassNameSeeder.php')) => rename($file, determineSeparator('./database/seeders/' . $className . 'Factory.php')),
        str_contains($file, determineSeparator('config/classname.php')) => rename($file, determineSeparator('./config/' . $packageSlugWithoutPrefix . '.php')),
        str_contains($file, determineSeparator('routes/classname.php')) => rename($file, determineSeparator('./routes/' . $packageSlugWithoutPrefix . '.php')),
        str_contains($file, determineSeparator('tests/ClassNameTest.php')) => rename($file, determineSeparator('./tests/' . $className . 'Test.php')),
